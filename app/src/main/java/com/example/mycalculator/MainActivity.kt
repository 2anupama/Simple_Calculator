...
class MainActivity : AppCompatActivity() {
    ...
    private fun onEqualsButtonClick() {
        ...
        try {
            ...
            showSnackbar("Result: $result", true)
        } catch (e: Exception) {
            editText.setText("Error: ${e.message}")
            showSnackbar("Error: Invalid Expression", false)
        }
    }
    ...
    private fun showSnackbar(message: String, isSuccess: Boolean) {
        val snackbar = Snackbar.make(findViewById(R.id.main_layout), message, Snackbar.LENGTH_SHORT)
        if (isSuccess) {
            snackbar.setBackgroundTint(ContextCompat.getColor(this, android.R.color.holo_green_light))
        } else {
